...
rule all:
    input:
        ...
        output_dir + "/organisms/SalmonIndex/seq.bin",
...
rule dexseq_prepare_annotation:
    ...
    output:
        ...
        gtf=output_dir + "/organisms/{organism}/Annotation/DEXSeq_flattened_exons.gtf"
    conda: "envs/basic.yaml"
    log: "logs/dexseq_prepare_{organism}.log"
    benchmark: "benchmarks/dexseq_prepare_{organism}.bmk.txt"
    shell:
        """
        python $(which dexseq_prepare_annotation.py) -f {output.gtf} -r no {input.gtf} {output.gff} > {log} 2>&1
