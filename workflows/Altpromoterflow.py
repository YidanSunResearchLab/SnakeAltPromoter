...
def main():
    ...
    parser.add_argument("--reference_condition", default="NA", help="Referece condition in differential analysis (e.g., --reference_condition 'SampleA'). Default: NA.")
    parser.add_argument("--method", type=str, default="all", choices=["salmon", "proactiv", "dexseq", "all"], help="Which method to run: salmon / proactiv / dexseq / all")
    ...
    def detect_samples(input_dir):
        ...
        for sample, files in samples_detected.items():
            ...
            if "2" not in files:
                print(f"Warning: Sample {sample} missing R2. Assuming single-end.")
