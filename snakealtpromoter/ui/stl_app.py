...
st.caption("UI interface for genome setup and main pipeline. ")
...
with tab1:
    ...
    fasta = st.text_input(
        "organism_fasta (path to genome.fa)", value="/full/path/to/your/genome.fa"
    )
    gtf = st.text_input(
        "genes_gtf (path to genes.gtf)", value="/full/path/to/your/genes.gtf"
    )
    main_chr = st.text_input(
        "main_chr_file (path to main chr list)", value="/full/path/to/your/main.chr.txt"
    )
    out_dir = st.text_input("Output directory (-o)", value="./genome")
    threads = st.number_input(
        "Threads (--threads)", min_value=1, max_value=128, value=30
    )
...
with tab2:
    ...
    input_dir = st.text_input(
        "-i Input FASTQ directory", value="/full/path/to/input/fastqs/dir/"
    )
    genome_dir = st.text_input(
        "--genome_dir (output from the previous step)",
        value="/full/path/to/genomesetup/dir/",
    )
    out_dir2 = st.text_input("-o Output directory", value="/full/path/to/output/dir/")
    ...
    extra = st.text_input(
        "Optional: extra advanced args to pass through verbatim", value=""
    )
