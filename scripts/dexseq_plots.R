...
# Salmon plots script
# Usage:
# Rscript dexseq_plots.R <output_dir> <promoter_rds> <activity> <gene_expr> <cell_lines> <condition>
# ---------------------
...
# Bar is divided to segments of level (Major, Minor, Inactive), each with promoter counts labelled.
  ...
  plot_cat_percentage <- function() {
    ...
    plot_activity_comparison <- function(){ 
        ...
        df2 <- rowData[rowData$geneClean %in% minor_genes, ]
        df2 <- rowData[!is.na(rowData[[col_class]]), ]
...
# The percentage of total promoter positions larger than 1 out of all promoter positions is labelled at the top of the bar.
  ...
  plot_genewise_percentage <- function() {
    # Group all promoters by geneClean. Each gene is now a list of promoter classes its promoters belong to.
    gene_split <- split(rowData[[col_class]], rowData$geneClean)
...
# Get promoter-to-gene mapping table: each row links a promoterId to its geneClean.
promoter_id_mapping <- promoterIdMapping(promoterAnnotationData)
# Remove version suffix from geneId to get geneClean.
promoter_id_mapping$geneClean <- sub("\\..*$", "", promoter_id_mapping$geneId)
# Remove duplicate promoter-gene pairs, keeping only unique pairs.
# Table count frequency each geneClean appears, representing the number of promoters associated with each geneClean.
# Create frequency table with each row representing geneClean and its frequency (promoter counts).
gene_prom_table <- as.data.frame(table(unique(promoter_id_mapping[, c("promoterId", "geneClean")])$geneClean))
